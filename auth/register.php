...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $statm = $conn->prepare("insert into users (name, username, email, password) values (:name, :username, :email, :password)");
        $statm->execute([
            'name' => $name,
            'username' => $username,
            'email' => $email,
        ...
        if ($statm->rowCount()) {
            header("Location: http://localhost/oop/oop-challenge/auth/login.php");
            return;
        }
        echo $e->getMessage();
